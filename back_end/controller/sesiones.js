...
export class SesionesController {
    constructor(sesionesModel, sesionesSchema) {
        this.sesionesModel = sesionesModel;
        this.sesionesSchema = sesionesSchema;
    ...
    getBy = async (req, res) => {
        try {
            ...
            const sesiones = await this.sesionesSchema.findAll(
                opcionesConsulta
            );
            ...
            res.status(500).json({
                type: "error",
                message: `Error al consultar las sesiones: ${error}`,
            });
        }
    };
    getAllBy = async (req, res) => {
        try {
            ...
            const resultadoConversaciones = await this.sesionesSchema.findAll(
                opcionesConsulta
            );
            // const conversacionesAgrupadas = {};
            // for (const mensaje of resultadoConversaciones) {
            //      if (!conversacionesAgrupadas[mensaje.sesion]) {
            //           conversacionesAgrupadas[mensaje.sesion] = [];
            //      }
            //      conversacionesAgrupadas[mensaje.sesion].push(mensaje);
            // }
            // const conversaciones = Object.keys(conversacionesAgrupadas).map(
            //      (id) => ({
            //           sesion: id,
            //           mensajes: conversacionesAgrupadas[id],
            //           cantidad_mensajes: conversacionesAgrupadas[id].length,
            //           id_negocio: conversacionesAgrupadas[id].id_negocio,
            //           nombre_negocio:
