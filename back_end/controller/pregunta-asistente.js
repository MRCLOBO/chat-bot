...
export class PreguntaAsistenteController {
    ...
    getAll = async (req, res) => {
        const preguntaAsistente = await this.preguntaAsistenteSchema.findAll();
    ...
    create = async (req, res) => {
        try {
                ...
                for (const nombreVariable of variables) {
                    ...
                    if (
                        !variablesUtilizadas.find(
                            (v) => v.nombre_variable_pregunta === nombreVariable
                        )
                    ) {
                        const variable =
            ...
            //   for (const variableEj of variablesUtilizadas) {
            //       const nombreVariable = variableEj.nombre_variable_pregunta;
            ...
            return res.status(200).json({
                type: "success",
                message:
                    "Pregunta de Asistente creada con éxito y sincronizada con Dialogflow.",
                bd: respuestaBD,
            });
        } catch (error) {
    ...
    delete = async (req, res) => {
        try {
            ...
            const projectPath = client.projectAgentPath(CREDENTIALS.project_id);
            ...
            res.json({
                type: "success",
                message:
                    "Pregunta de asistente e intent eliminados correctamente.",
            });
        } catch (error) {
    ...
    update = async (req, res) => {
        try {
            ...
            return res.json({
                type: "success",
                message:
                    "Pregunta de asistente actualizada y sincronizada con Dialogflow.",
            });
        } catch (error) {
    ...
    async obtenerUltimoID() {
        try {
            const ultimoRegistro = await this.preguntaAsistenteSchema.findOne({
        ...
        while ((match = regex.exec(texto)) !== null) {
            ...
            const valorEjemplo =
                valoresEjemplo[varName][
                    Math.floor(Math.random() * valoresEjemplo[varName].length)
                ] || varName;
    ...
    async obtenerCredencialesDelNegocio(id_negocio) {
        ...
        if (!negocio || !negocio.api_key) {
            throw new Error("No se encontraron las credenciales del negocio.");
        }
