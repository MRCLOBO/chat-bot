...
import registerSockets from './socket/socket.js';
...
import { CursoModel, CursoSchema } from './models/curso.js';
import { createCursoRouter } from './routes/curso.js';
import { MateriaModel, MateriaSchema } from './models/materia.js';
...
import { AuditoriaModel, AuditoriaSchema } from './models/auditoria.js';
...
const io = initIO(server); // Instancia singleton de Socket.IO
...
app.get('/', (req, res) => {
     res.send('HELPI FUNCIONANDO');
});

//Para saber cada vez que se reciba una solicitud en la API
app.use(async (req, res, next) => {
     console.log('request recibido el ', new Date().toString());
     if (!req.headers['x-apodo']) return next(); // no hacemos nada si no hay usuario

     const t = await sequelize.transaction();

     await sequelize.query(
          `SET "app.current_user" = '${req.headers['x-apodo']}'`,
          { transaction: t }
     );

     // guardamos la transacción para que tus servicios la usen
     req.dbTransaction = t;

     // commit o rollback automático cuando termina la request
     res.on('finish', async () => {
          try {
               await t.commit();
          } catch (err) {
...
//consultas con respecto a las carreras registradas
app.use('/curso', createCursoRouter(CursoModel, CursoSchema));

//consultas con respecto a las materias registradas
app.use('/materia', createMateriaRouter(MateriaModel, MateriaSchema));

//consultas con respecto a las notas registradas
app.use('/nota', createNotaRouter(NotaModel, NotaSchema));

//consultas con respecto a auditoria informatica
app.use('/auditoria', createAuditoriaRouter(AuditoriaModel, AuditoriaSchema));
